...
return [
    ...
    'Errors logged' => 'Geregistreerde fouten',
    'Please check the <a href="{{ extra.url }}">error log</a> to see if there are any serious issues.' => 'Controleer het <a href="{{ extra.url }}">foutlogboek</a> om te zien of er ernstige problemen zijn.',
    'Regular' => 'Regulier',
    'No modules are installed.' => 'Er zijn geen modules geïnstalleerd.',
    'forms' => 'formulieren',
    'form' => 'formulier',
    '{{ value != 1 ? "forms" : "form" }}' => '{{ value != 1 ? "formulieren" : "formulier" }}',
    '{{ value == 1 ? "Enabled" : "Disabled" }}' => '{{ value == 1 ? "Engeschakeld" : "Uitgeschakeld" }}',
    '{{ value ? "Enabled" : "Disabled" }}' => '{{ value ? "Engeschakeld" : "Uitgeschakeld" }}',
    '{{ value.enabled ? "Enabled" : "Disabled" }}' => '{{ value.enabled ? "Engeschakeld" : "Uitgeschakeld" }}',
    '{{ value ? "Enabled" : "Not found" }}' => '{{ value ? "Engeschakeld" : "Niet gevonden" }}',
    '{{ value ? value : "Not set" }}' => '{{ value ? value : "Niet ingesteld" }}',
    '{{ value ? value~" errors found" : "None found" }}' => '{{ value ? value~" fouten gevonden" : "Geen gevonden" }}',
    '{{ value.enabled ? "Enabled, "~value.interval~" days" : "Disabled" }}' => '{{ value.enabled ? "Engeschakeld, "~value.interval~" dagen" : "Uitgeschakeld" }}',
    '{{ value.enabled ? "Enabled, "~value.interval~" hours" : "Disabled" }}' => '{{ value.enabled ? "Engeschakeld, "~value.interval~" uur" : "Uitgeschakeld" }}',
    '{{ value.enabled ? "Enabled, "~value.interval~" seconds" : "Disabled"  }}' => '{{ value.enabled ? "Engeschakeld, "~value.interval~" seconden" : "Uitgeschakeld"  }}',
    '{{ value.enabled ? "Enabled, "~value.interval~" minutes" : "Disabled"  }}' => '{{ value.enabled ? "Engeschakeld, "~value.interval~" minuten" : "Uitgeschakeld"  }}',
    '{% if value.count != 0 %}{{ value.count }} per {{ value.interval == "m" ? "minute" : "second"  }}{% else %}Unlimited{% endif %}' => '{% if value.count != 0 %}{{ value.count }} per {{ value.interval == "m" ? "minuut" : "seconde"  }}{% else %}Onbeperkt{% endif %}',
